...
glimpse(mtcars.data)
```
By glimpsing the dataframe, we notice that the 'displacement.per.cylinder'
column is present, but is stored as a dataframe that is not easily viewed. Upon
examining the object itself, all values are present in the table, but it must
be opened manually to view them. Any version of glimpse or print will simply 
list the column row location is filled by a dataframe.

To get around this issue, we can use the 'tidyverse' alternative. Below, we tell
R to, once again, store mtcars into our data. This time, we create a new 
dataframe, termed 'tidyverse_disp_per_cyl', that is mutated by the addition of a
column that has the values of displacement divided by the number of cylinders
added to it. 
```{r}
mtcars <- mtcars
...
```
Via this method, we store the actual value into this dataframe, 
instead of creating a dataframe within a dataframe.
